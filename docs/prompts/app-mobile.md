...
- `/trilhas` — as **17 trilhas** agrupadas por plano (ver lista na seção 6), com cadeado por tier.
...
- `/exercicios` — **+5.000 exercícios** (tabela `practice_exercises`, 81 categorias de todo o ecossistema) com busca/filtros por categoria/nível e **correção por IA** (nota, feedback, melhor solução) + fallback se IA falhar.
- `/ide` — **IDE Python** (Pyodide via WebView), rodando Python no aparelho; temas, snippets e "Salvar no GitHub" (cria repo). Conectada aos exercícios.
...
- `/entrevista-ia` (Suprema) — simulador de entrevista técnica com IA (perguntas + avaliação das respostas).
- `/curriculo` (Completo+) — **gerador de currículo**: editor completo (dados, resumo, skills, experiência, formação, projetos, idiomas), 8 templates (4 Suprema-only), preview ao vivo, exportar PDF/DOCX/DOC/TXT, CRUD salvo em `resumes` (RLS).
- `/extensao` (apresenta a **extensão oficial do VS Code**, exclusiva do Suprema) — instalar pela Marketplace ou baixar o `.vsix` (download gated p/ Suprema via `/api/extension/download`).

**Conta**
- `/perfil` — perfil avançado: capa, avatar, headline, skills, bio, links (GitHub/LinkedIn/site) + contadores sociais (conexões/seguidores/seguindo) clicáveis → `/comunidade/conexoes`.
- `/notificacoes` — lista de notificações (plano, comunidade, suporte, nível) com badge **vermelho** de não-lidas no menu; marca como lidas ao abrir. Tabela `notifications` (RLS own).
- `/configuracoes` (hub) com subrotas: `conta`, `perfil`, `plano` (upgrade/downgrade/reembolso/portal via WebBrowser), `ia` (BYOK), `api` (chaves da API da plataforma — **Suprema**, com playground), `github` (PAT p/ criar repos), `indicacoes`, `seguranca` (2FA + dispositivos confiáveis), `aparencia` (tema), `notificacoes`, `plataforma`, `dados` (exportar/excluir conta — LGPD).
- `/suporte` — enviar mensagem ao admin + thread realtime (presença/online do admin).

**Tradução**: o app deve suportar pelo menos PT (padrão); a web traduz para EN, ES, ZH e KO. No app, manter PT e, se viável, i18n para os mesmos idiomas.

**Onboarding/Tutorial**: no 1º acesso (coluna `users_profile.tutorial_done`), mostrar um tutorial guiado que leva às rotas; não reexibir depois. Banner de **consentimento de cookies/privacidade** equivalente na 1ª abertura (LGPD), salvo localmente.
...
- `/admin/clientes` — métricas (MRR, ARR, **saldo/pendente/próximas cobranças** via Stripe `payment_intents`/`balance`), lista de clientes (excluir/mudar plano), busca, CSV. **Excluir os próprios admins do faturamento.**
- `/admin/moderacao` — denúncias pendentes (remover post / bloquear autor / resolver) + bloqueados.
- `/admin/mensagens` — caixa de entrada do suporte + responder (realtime).
- `/admin/seo` — SEO/Google (score, preview, analytics de usuários).
- `/admin/extensao` — gerenciar a extensão do VS Code (upload `.vsix`, versão, URL da Marketplace, changelog).
- `/admin/taxas`, `/admin/banco`, `/admin/traducao`, `/admin/trustmrr`.

> **Stripe (importante):** os planos pagos têm **7 dias de trial** (`STRIPE_TRIAL_DAYS=7`) — o usuário ativa o plano mas só é cobrado no fim do trial. A chave Stripe é **restrita** (`rk_live_`): no app/painel, ler receita via `payment_intents` (não exige permissão de `charges`).
...
## 10. Critérios de aceite (Deve conter tudo que esta na plataforma)
...
- [ ] 17 trilhas + leitor de conteúdo + IDE Python funcionando.
...
## 11. Funcionalidades adicionais a refletir (paridade com a web — jun/2026)

- **Notificações push + in-app**: `expo-notifications` para streak/lembretes; lista em `/notificacoes` lendo `notifications` (badge de não-lidas). Eventos: subiu de nível, comentário/curtida na comunidade, resposta do suporte, **plano cancelado**, **3 dias antes da 1ª cobrança** (fim do trial), reembolso.
- **E-mails transacionais** (já no backend, não no app): boas-vindas pós-confirmação, aviso de fim do trial, cancelamento, reembolso — o app só reflete as notificações in-app correspondentes.
- **Gerador de currículo** (`/curriculo`, Completo+): editor + preview; exportação de PDF/DOCX no mobile via share/print (`expo-print`/`expo-sharing`) ou abrir no site. CRUD em `resumes`.
- **Extensão VS Code** (`/extensao`): tela informativa + botão "Instalar no VS Code" (Marketplace) e, para Suprema, "Baixar .vsix".
- **Entrevista com IA** (`/entrevista-ia`, Suprema) e **API da plataforma** (`/configuracoes/api`, Suprema, com playground e geração de `pytk_live_` keys).
- **Comunidade completa**: feed com Markdown (preview), vagas (categoria `vaga`), comentários (corrigidos), curtidas, perfis clicáveis, conexões/seguidores (`/comunidade/conexoes`), denúncias.
- **Perfil social**: capa, skills, contadores sociais clicáveis.
- **Gamificação**: XP/nível (`community_profiles`/`users_profile`), ranking, streak.
- **Acessibilidade + i18n** (PT/EN/ES/ZH/KO), tema claro/escuro, **consentimento de cookies/privacidade** na 1ª abertura, **tutorial** no 1º acesso (`tutorial_done`).

### Endpoints HTTPS do site reutilizáveis pelo app (além do Supabase direto)
- `POST /api/extension/login`, `GET /api/extension/sync` (referência de padrão de auth por Bearer).
- `GET /api/v1/me`, `/api/v1/progress`, `/api/v1/tracks`, `/api/v1/ranking` (API pública c/ `pytk_live_` — útil para widgets).
- Para IA: preferir **Supabase Edge Function** ou endpoint do site que encapsula OpenRouter/NVIDIA (BYOK via `user_ai_settings`/`api_keys`). **Nunca** colocar chave de IA no bundle.

## 12. Critérios de aceite adicionais

- [ ] `/notificacoes` com badge de não-lidas; push de streak/lembrete.
- [ ] `/curriculo` (editor + export), `/entrevista-ia`, `/extensao`, `/configuracoes/api` (Suprema).
- [ ] +5.000 exercícios com busca/filtros e correção por IA.
- [ ] Comunidade completa (feed Markdown, vagas, comentários, conexões, denúncias).
- [ ] Trial de 7 dias refletido (ativa plano, cobra depois); receita lida via `payment_intents`.
- [ ] Tutorial + consentimento de cookies na 1ª abertura; i18n PT (mín.).
