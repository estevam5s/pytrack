...
const DOMAINS = [
  "chamados técnicos", "equipamentos", "manutenções", "garantias", "peças", "ordens de serviço",
  "salas de aula", "bibliotecas", "acervos", "empréstimos", "multas", "associados",
];
...
for (const [group, level, type, taskTemplates] of TASKS) {
  for (const tpl of taskTemplates) {
    for (const d of DOMAINS) {
      ...
      n += 1;
      out.push({
        ex_id: `MX${String(n).padStart(5, "0")}`,
        title,
        category: group,
        group_label: group,
        level,
