...
const DOMAINS = [
  "produtos", "usuários", "pedidos", "eventos", "tarefas", "clientes", "faturas", "estoque",
  "reservas", "mensagens", "posts", "comentários", "transações", "cursos", "alunos", "professores",
  "veículos", "imóveis", "playlists", "livros", "filmes", "receitas", "sensores", "tickets de suporte",
];
...
for (const [group, level, type, taskTemplates] of TASKS) {
  for (const tpl of taskTemplates) {
    for (const d of DOMAINS) {
      ...
      n += 1;
      out.push({
        ex_id: `JX${String(n).padStart(5, "0")}`,
        title,
        category: group,
        group_label: group,
        level,
