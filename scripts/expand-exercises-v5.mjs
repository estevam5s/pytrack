...
const DOMAINS = [
  "carteiras digitais", "criptomoedas", "ações da bolsa", "jogos", "torneios", "academias", "treinos",
  "consultas médicas", "exames", "medicamentos", "voos", "hotéis", "aluguéis de carro", "entregas",
  "rotas de transporte", "pesquisas", "enquetes", "doações", "voluntários", "ONGs", "wikis", "glossários",
  "dispositivos IoT", "leituras de sensores", "alertas",
];
...
for (const [group, level, type, taskTemplates] of TASKS) {
  for (const tpl of taskTemplates) {
    for (const d of DOMAINS) {
      ...
      n += 1;
      out.push({
        ex_id: `LX${String(n).padStart(5, "0")}`,
        title,
        category: group,
        group_label: group,
        level,
