...
export async function createComment(input: {
  ...
  // garante o perfil de comunidade (não deixa a RPC quebrar o comentário)
  try {
    await supabase.rpc("community_ensure_profile", { uid: user.id });
  } catch {
    /* segue mesmo se a RPC falhar */
  }
...
export async function createJob(input: {
  ...
  // também publica no feed (category 'vaga') para a vaga aparecer para todos
  const parts = [
    `💼 **${input.title.trim()}** — ${input.company.trim()}`,
    [input.seniority, input.contract_type, input.remote ? "Remoto" : input.location]
      .filter(Boolean)
      .join(" · "),
    input.salary_range ? `Faixa: ${input.salary_range}` : "",
    input.description ?? "",
    input.apply_url ? `Candidatar-se: ${input.apply_url}` : "",
  ].filter(Boolean);
  await supabase.from("community_posts").insert({
    user_id: user.id,
    category: "vaga",
    title: `Vaga: ${input.title.trim()} @ ${input.company.trim()}`,
    content: parts.join("\n\n").slice(0, 5000),
    tags: (input.tags ?? []).slice(0, 8),
    visibility: "public",
  });
