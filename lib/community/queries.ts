...
async function authorMap(userIds: string[]) {
  ...
  const [{ data }, { data: profiles }] = await Promise.all([
    supabase.from("community_profiles").select("*").in("user_id", ids),
    // fallback: avatar/nome reais ficam em users_profile (community_profiles fica NULL)
    supabase.from("users_profile").select("user_id, name, avatar_url").in("user_id", ids),
  ]);
  const byUser = new Map(
    (profiles ?? []).map((p) => [p.user_id, p as { name: string | null; avatar_url: string | null }]),
  );
  return new Map(
    (data ?? []).map((p) => {
      const up = byUser.get(p.user_id);
      return [
        p.user_id,
        {
          ...p,
          avatar_url: p.avatar_url ?? up?.avatar_url ?? null,
          display_name: p.display_name ?? up?.name ?? p.display_name,
        } as CommunityProfile,
      ];
    }),
  );
...
export async function getFeed(
  ...
  else if (filter === "meus" && user) query = query.eq("user_id", user.id);
