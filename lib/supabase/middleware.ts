...
export async function updateSession(request: NextRequest) {
  ...
  // a menos que este dispositivo seja "confiável" (cookie válido por 30 dias).
  if (user && isProtected) {
    try {
      const { data: aal } =
        await supabase.auth.mfa.getAuthenticatorAssuranceLevel();
      if (aal?.currentLevel === "aal1" && aal?.nextLevel === "aal2") {
        const trusted = request.cookies.get("pytrack_td")?.value;
        const ok = trusted ? await isTrustedDevice(trusted, user.id) : false;
        if (!ok) {
          const url = request.nextUrl.clone();
