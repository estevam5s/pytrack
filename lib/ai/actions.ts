 ...
 *  Aplica quota diária por plano (quem usa BYOK não tem limite). */
async function aiJson<T>(messages: ChatMessage[], opts: ChatOptions = {}) {
  const override = await getUserAiOverride();
  const quota = await consumeAiQuota(Boolean(override?.apiKey));
  if (!quota.allowed) {
    return {
      data: null,
      error: `Você atingiu o limite diário de ${quota.limit} usos de IA do seu plano. Faça upgrade ou configure sua própria chave em Configurações → IA.`,
      model: undefined,
    } as Awaited<ReturnType<typeof chatJson<T>>>;
  }
...
export async function analyzeProject(input: {
  ...
  if (!input.code.trim()) {
    return { ...empty, error: "Cole o código do seu projeto antes de avaliar." };
  }

  const res = await aiJson<Partial<ProjectAnalysis>>(
    [
      {
        role: "system",
        content:
          "Você é um revisor sênior. Avalie a solução de um PROJETO de programação Python frente aos requisitos. " +
          "Responda SOMENTE com JSON válido com as chaves: score (inteiro 0-10), meets_requirements (boolean), " +
          "summary (string em pt-BR), strengths (array de strings), improvements (array de strings com melhorias concretas). " +
          "Seja honesto, técnico e construtivo. Escape aspas e quebras de linha nas strings.",
      },
      {
        role: "user",
        content:
    ...
    { maxTokens: 1100, temperature: 0.25 },
  );

  const p = res.data;
  if (!p) {
    return { ...empty, error: res.error ?? "Não consegui interpretar a resposta da IA.", model: res.model };
  }
  const arr = (v: unknown) => (Array.isArray(v) ? v.map(String) : []);
  return {
    score: Number(p.score ?? 0),
    meets_requirements: Boolean(p.meets_requirements),
    summary: String(p.summary ?? ""),
    strengths: arr(p.strengths),
