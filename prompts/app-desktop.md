...
**Estudar:** `/inicio` · `/trilhas` (17) · `/trilhas/:id` · `/conteudos/:modulo` · `/conteudos/:modulo/:licao` · `/comunidade` (Completo+: feed Markdown, vagas, comentários, conexões/seguidores, denúncias) · `/evolucao` · `/stack` · `/exercicios` (+5.000, 81 categorias, busca/filtros + correção IA) · `/ide` (Pyodide + temas/snippets + Salvar no GitHub)

**Recursos:** `/aulas-udemy` · `/aulas-youtube` · `/material` · `/livros` · `/aplicativo`

**Carreira:** `/minha-carreira` · `/especializacoes` · `/consultor-ia` · `/vagas` · `/perguntas-carreira-python` · `/entrevista-ia` (Suprema, simulador) · `/curriculo` (Completo+, editor + 8 templates + export PDF/DOCX/DOC/TXT, CRUD em `resumes`) · `/extensao` (extensão VS Code, Suprema — instalar/baixar .vsix)

**Conta:** `/perfil` (capa/skills/contadores sociais → `/comunidade/conexoes`) · `/notificacoes` (badge vermelho de não-lidas, tabela `notifications`) · `/configuracoes` (+ `conta`, `perfil`, `plano`, `ia` BYOK, `api` Suprema+playground, `github` PAT, `indicacoes`, `seguranca` 2FA+dispositivos, `aparencia`, `notificacoes`, `plataforma`, `dados` LGPD) · `/suporte` (realtime + presença admin)
...
## 11. Funcionalidades adicionais (paridade com a web — jun/2026)

- **Notificações nativas + in-app**: lembretes/streak via notificação nativa do SO; `/notificacoes` lê `notifications` (badge de não-lidas na sidebar). Eventos: subiu de nível, comentário/curtida, resposta do suporte, **plano cancelado**, **3 dias antes da 1ª cobrança** (fim do trial), reembolso.
- **Gerador de currículo** (`/curriculo`, Completo+): editor + preview + export PDF/DOCX/DOC/TXT (no desktop dá para gerar PDF nativo/print) + CRUD em `resumes`.
- **Extensão VS Code** (`/extensao`), **Entrevista com IA** (`/entrevista-ia`, Suprema), **API da plataforma** (`/configuracoes/api`, Suprema, com playground e geração de `pytk_live_`).
- **Comunidade completa** e **perfil social** (capa/skills/contadores), **gamificação** (XP/nível/ranking/streak).
- **i18n** PT/EN/ES/ZH/KO, **consentimento de cookies/privacidade** na 1ª abertura, **tutorial** no 1º acesso (`users_profile.tutorial_done`).
- **Stripe**: planos pagos têm **7 dias de trial** (`STRIPE_TRIAL_DAYS=7`) — ativa plano, cobra ao fim do trial. Chave **restrita** (`rk_live_`): ler receita por `payment_intents` no painel admin (sem permissão de `charges`).

### Endpoints HTTPS reutilizáveis
- `GET /api/v1/me`, `/api/v1/progress`, `/api/v1/tracks`, `/api/v1/ranking` (API pública c/ `pytk_live_`).
- IA via Supabase Edge Function ou endpoint do site (encapsula OpenRouter/NVIDIA, BYOK `user_ai_settings`/`api_keys`). **Nunca** chave de IA no app.

## 12. Distribuição (igual ao mobile)

- Os instaladores (.msi/.exe, .dmg, .AppImage/.deb) vão para a rota **`/aplicativo`** (tabela `app_releases`, `platform` = `windows`/`macos`/`linux`). Como o **Supabase free limita 50MB/arquivo**, hospedar instaladores grandes em **GitHub Releases** (`estevam5s/pytrack`) e registrar a `download_url` em `app_releases`. O APK Android já está lá (referência do fluxo).
- **GitHub Actions** (matriz windows/macos/ubuntu) faz o build cross-OS e anexa os artefatos na release.

## 13. Critérios de aceite adicionais

- [ ] `/notificacoes` (badge), `/curriculo`, `/entrevista-ia`, `/extensao`, `/configuracoes/api` (Suprema).
- [ ] +5.000 exercícios com busca/filtros e correção IA; comunidade completa.
- [ ] Trial de 7 dias refletido; receita via `payment_intents`.
- [ ] Tutorial + cookies na 1ª abertura; i18n PT (mín.).
- [ ] Instaladores nas 3 plataformas publicados em `/aplicativo` (via GitHub Releases).
