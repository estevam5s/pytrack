...
export function ClientsDashboard({ kpis, monthly, planDistribution, stripe, customers, sources = [] }: Props) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="card overflow-hidden">
          ...
          <div className="grid gap-4 p-5 sm:grid-cols-4">
            <div>
              ...
              <p className={cn("mt-1 text-xl font-bold", stripe.available < 0 ? "text-red-400" : "text-green")}>{fmtBRL(stripe.available)}</p>
              {stripe.available < 0 && <p className="text-[10px] text-text-secondary">a recuperar (reembolso)</p>}
            </div>
            <div>
              ...
              {stripe.pendingAvailableOn && (
                <p className="text-[10px] text-text-secondary">libera em {new Date(stripe.pendingAvailableOn).toLocaleDateString("pt-BR")}</p>
              )}
            </div>
            <div>
              ...
              <p className="text-[10px] text-text-secondary">{stripe.chargesCount} pagamentos</p>
            </div>
            <div>
              <p className="flex items-center gap-1.5 text-xs text-text-secondary"><Receipt className="h-3.5 w-3.5" /> Próximas cobranças</p>
              <p className="mt-1 text-xl font-bold text-primary-light">{fmtBRL(stripe.upcoming?.total ?? 0)}</p>
              <p className="text-[10px] text-text-secondary">
                {stripe.upcoming?.count ?? 0} assinaturas
                {stripe.upcoming?.nextDate ? ` · próx. ${new Date(stripe.upcoming.nextDate).toLocaleDateString("pt-BR")}` : ""}
              </p>
            </div>
          </div>
        </div>
      ...
      {sources.length > 0 && (
        <div className="card p-5">
          <p className="mb-4 text-sm font-semibold">Origem dos cadastros</p>
          <div className="space-y-2.5">
            {(() => {
              const max = Math.max(1, ...sources.map((s) => s.count));
              return sources.map((s) => (
                <div key={s.source} className="flex items-center gap-3">
                  <span className="w-36 shrink-0 truncate text-right text-xs font-medium text-text-secondary">{s.source}</span>
                  <div className="relative h-6 flex-1 overflow-hidden rounded-md bg-surface-2">
                    <div
                      className="absolute inset-y-0 left-0 rounded-md bg-gradient-to-r from-primary to-primary-light"
                      style={{ width: `${Math.max((s.count / max) * 100, 6)}%` }}
                    />
                    <span className="absolute inset-y-0 right-2 flex items-center text-[11px] font-semibold">{s.count}</span>
                  </div>
                </div>
              ));
            })()}
      ...
      <div className="card overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-sm">
            ...
            <tbody>
              {filtered.map((c, i) => (
                <tr key={c.userId + i} className={cn("border-t border-border transition-colors hover:bg-surface-2/50", i % 2 && "bg-surface/40")}>
                  <td className="p-3">
                    <div className="flex items-center gap-2.5">
                      {c.avatarUrl ? (
                        ...
                        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/15 text-xs font-bold text-primary-light">
                          {(c.name ?? c.email).charAt(0).toUpperCase()}
                        </span>
                      )}
                      <div className="min-w-0">
                        {c.name && <p className="truncate text-sm font-medium">{c.name}</p>}
                        <p className="max-w-[200px] truncate text-xs text-text-secondary">{c.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-3">
