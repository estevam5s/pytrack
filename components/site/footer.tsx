...
import { FooterSnake } from "./footer-snake";
...
const COLS = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: "API da plataforma", href: "/docs/api" },
      { label: "Apps & Downloads", href: "/apps" },
      { label: "Bot do Telegram", href: "/bot" },
      { label: "GitHub", href: "https://github.com/PyTrackOrganization" },
  ...
  },
  {
    ...
    links: [
      ...
      { label: "Segurança", href: "/.well-known/security.txt" },
  ...
  return (
    <footer className="relative mt-10 overflow-hidden border-t border-border bg-surface">
      ...
      <div className="container relative py-14">
        <div className="grid gap-x-10 gap-y-10 lg:grid-cols-[1.5fr_repeat(4,1fr)]">
          <Reveal className="max-w-xs">
            ...
            {/* baixe os apps */}
            <p className="mt-6 text-xs font-semibold uppercase tracking-wide text-text-secondary">Baixe os apps</p>
            <div className="mt-2.5 flex flex-wrap gap-2">
              <Link href="/apps" className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-surface px-3 py-1.5 text-xs text-text-secondary transition-colors hover:border-primary/40 hover:text-foreground">
              ...
              <Link href="/apps" className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-surface px-3 py-1.5 text-xs text-text-secondary transition-colors hover:border-primary/40 hover:text-foreground">
                <Puzzle className="h-3.5 w-3.5" /> VS Code
              </Link>
            </div>

            {/* status do sistema */}
            <Link href="/status" className="mt-5 inline-flex items-center gap-2 rounded-full border border-green/30 bg-green/5 px-3 py-1.5 text-xs text-green transition-opacity hover:opacity-80">
          ...
          {/* animação decorativa (cobra Python devorando código) — preenche o vazio do meio, só desktop */}
          <div className="hidden self-center lg:col-start-2 lg:col-span-4 lg:block">
            <FooterSnake />
          </div>
        </div>
