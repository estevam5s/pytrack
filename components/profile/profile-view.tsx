...
export function ProfileView({
  ...
  social?: { followers: number; following: number; connections: number };
}) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <div
          className="h-32 bg-gradient-to-r from-primary/30 via-primary/10 to-secondary/20 bg-cover bg-center sm:h-44"
        ...
        <CardContent className="p-5 sm:p-6">
          ...
          {/* contadores sociais (clicáveis → minha rede) */}
          {social && (
            <div className="mt-4 flex flex-wrap items-center gap-2 border-y border-border py-3">
              {[
                ["Conexões", social.connections],
                ["Seguidores", social.followers],
                ["Seguindo", social.following],
              ].map(([label, value]) => (
                <Link
                  key={label as string}
                  href="/comunidade/conexoes"
                  className="rounded-lg px-3 py-1 text-center transition-colors hover:bg-surface-2"
                >
              ...
              <Link
                href="/comunidade/conexoes"
                className="ml-auto inline-flex items-center gap-1.5 self-center text-sm font-medium text-primary-light hover:underline"
              >
                Ver minha rede
              </Link>
            </div>
          ...
          {profile.skills && profile.skills.length > 0 && (
            <div className="mt-4">
              <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-text-secondary">Habilidades</p>
              <div className="flex flex-wrap gap-1.5">
                {profile.skills.map((s) => (
                  <span key={s} className="rounded-full border border-primary/30 bg-primary/10 px-2.5 py-0.5 text-xs text-primary-light">
                    {s}
                  </span>
                ))}
              </div>
