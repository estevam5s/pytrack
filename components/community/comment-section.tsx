...
function CommentForm({
  ...
  const submit = () => {
    ...
    setError(null);
    // guarda otimista do texto e limpa o campo já
    const text = value;
    start(async () => {
      const res = await createComment({ postId, content: text, parentId });
      if (res?.error) {
        setError(res.error);
      } else {
        setValue("");
  ...
  return (
    <div className={cn(small ? "" : "mt-1")}>
      ...
      {error && <p className="mt-1 pl-10 text-xs text-red-400">{error}</p>}
    </div>
  );
}
