    ...
    <Card hover className="group flex h-full flex-col overflow-hidden">
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-gradient-to-br from-primary/15 via-surface to-secondary/10">
        ...
        <div className="absolute left-2 top-2">
          <Badge className={levelColor(book.level)}>
        ...
        {owned ? (
          <div className="absolute right-2 top-2 flex gap-1.5 opacity-0 transition-opacity group-hover:opacity-100">
            <button
              onClick={onEdit}
              className="rounded-md bg-background/80 p-1.5 backdrop-blur hover:bg-primary hover:text-white"
              aria-label="Editar"
            >
            ...
            <button
              onClick={onDelete}
              className="rounded-md bg-background/80 p-1.5 backdrop-blur hover:bg-danger hover:text-white"
              aria-label="Remover"
            >
          ...
          <span className="absolute right-2 top-2 rounded-full bg-background/80 px-2 py-0.5 text-[10px] font-medium text-text-secondary backdrop-blur">
            Recomendado
          </span>
        )}
      </div>
          ...
          {book.file_url ? (
            <Button asChild size="sm" variant="outline" className="flex-1">
              <a href={book.file_url} target="_blank" rel="noopener noreferrer">
                <Download className="h-3.5 w-3.5" /> Ler
              </a>
            </Button>
          ) : book.url ? (
            <Button asChild size="sm" variant="outline" className="flex-1">
      ...
      <Dialog open={!!deleting} onOpenChange={(v) => !v && setDeleting(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              &quot;{deleting?.title}&quot; será removido da sua biblioteca.
            </DialogDescription>
          </DialogHeader>
