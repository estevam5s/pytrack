...
export function UdemyView({ courses }: { courses: UdemyCourse[] }) {
  ...
  const stats = useMemo(() => {
    ...
    // tokeniza categorias/stack (campos com várias techs separadas por vírgula)
    // em techs individuais → ranking legível em vez de labels gigantes.
    const byTech = new Map<string, number>();
    for (const c of courses) {
      const raw = (c.category || "").split(/[,;/|]+/).map((t) => t.trim()).filter(Boolean);
      const list = raw.length ? raw : ["Sem categoria"];
      for (const t of list) {
        const key = t.length > 24 ? t.slice(0, 24) + "…" : t;
        byTech.set(key, (byTech.get(key) ?? 0) + 1);
      }
    }
    const topCount = Math.max(1, ...byTech.values());
    const categories = [...byTech.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      .map(([area, n]) => ({ area, count: n, percentage: Math.round((n / topCount) * 100) }));
                ...
                <div className="space-y-2.5">
                  {stats.categories.map((c, i) => (
                    <div key={c.area} className="flex items-center gap-3">
                      <span className="w-40 shrink-0 truncate text-right text-xs font-medium text-text-secondary" title={c.area}>
                        {c.area}
                      </span>
                      <div className="relative h-6 flex-1 overflow-hidden rounded-md bg-surface-2">
                        <div
                          className="absolute inset-y-0 left-0 rounded-md bg-gradient-to-r from-primary to-primary-light transition-all"
                        ...
                        <span className="absolute inset-y-0 right-2 flex items-center text-[11px] font-semibold text-foreground">
                          {c.count} {c.count === 1 ? "curso" : "cursos"}
                        </span>
                      </div>
                    </div>
                  ))}
