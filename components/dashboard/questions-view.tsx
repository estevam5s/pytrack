...
function QuestionCard({
  ...
  return (
    <Card className={cn(studied && "border-secondary/40 bg-secondary/[0.03]")}>
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <CardContent className="space-y-4 border-t border-border pt-4">
          {q.intro && (
            <p className="text-sm text-text-secondary">{q.intro}</p>
          )}
          ...
          <Button
            size="sm"
            variant={studied ? "secondary" : "outline"}
            onClick={onToggle}
          >
