...
export async function requestRefund(
  ...
  try {
    ...
    // e-mail + notificação de reembolso
    try {
      const { sendEmail } = await import("@/lib/email");
      const { refundEmailTemplate } = await import("@/lib/email-templates");
      const name = (user.user_metadata?.name as string | undefined)?.split(" ")[0] ?? "";
      await admin.from("notifications").insert({
        user_id: user.id,
        type: "plan",
        title: "Reembolso processado",
        body: "Seu reembolso foi processado. O valor retorna em alguns dias úteis.",
        link: "/configuracoes/plano",
      });
      if (user.email) {
        await sendEmail({
          to: user.email,
          subject: "Reembolso processado · PyTrack",
          text: `Olá${name ? `, ${name}` : ""}. Seu reembolso foi processado com sucesso e seu acesso premium foi encerrado. Não haverá novas cobranças.`,
          html: refundEmailTemplate(name),
        });
      }
    } catch {
      /* não bloqueia o reembolso se o e-mail falhar */
    }
