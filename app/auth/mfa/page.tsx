...
import { trustThisDevice } from "./actions";
...
export default function MfaPage() {
  ...
  // auto-envia quando completar 6 dígitos
  useEffect(() => {
    if (code.length === 6 && factorId && !busy) {
      verify();
    }
  ...
  async function verify() {
    ...
    if (v.error) {
      setBusy(false);
      return setErr("Código inválido. Tente novamente.");
    }
    // lembra este dispositivo por 30 dias (não pede o código de novo)
    if (remember) {
      try {
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-background px-6">
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-border bg-surface p-7 text-center">
        ...
        <label className="mt-3 flex cursor-pointer items-center justify-center gap-2 text-xs text-text-secondary">
          <input
            type="checkbox"
            checked={remember}
