...
export function PythonIDE({ fullscreen = false }: { fullscreen?: boolean }) {
  ...
  const [panel, setPanel] = useState<"explorer" | "examples" | "search" | "outline" | "snippets" | "ssh" | "settings">("examples");
  ...
  const [sshBridge, setSshBridge] = useState("");
  const filesRef = useRef<{ name: string; code: string }[]>([]);
  ...
  // mantém filesRef sincronizado (usado por atalhos e pelo terminal) + carrega bridge SSH
  useEffect(() => { filesRef.current = files; }, [files]);
  useEffect(() => {
    try { setSshBridge(localStorage.getItem("pytrack-ssh-bridge") || ""); } catch { /* ignore */ }
  }, []);
...
`);
  ...
  // Autoformatação: usa black (via micropip) com fallback para um normalizador simples.
  const formatCode = useCallback(async (src: string): Promise<string> => {
    try {
      const py = await getPyodide();
      try { await py.runPythonAsync("import black"); }
      catch {
        await py.runPythonAsync("import micropip").catch(async () => { await py.loadPackage("micropip"); await py.runPythonAsync("import micropip"); });
        await py.runPythonAsync("await micropip.install('black')");
      }
      py.globals.set("__src", src);
      const out = await py.runPythonAsync(
        "import black\nblack.format_str(__src, mode=black.Mode(line_length=88))",
      );
  ...
  // Formata o arquivo ativo (atalho ⇧⌘F / botão)
  const doFormat = useCallback(async () => {
    setFormatting(true);
    try {
      const src = filesRef.current[active]?.code ?? code;
      const out = await formatCode(src);
      setCode(out);
      setFiles((fs) => fs.map((f, i) => (i === active ? { ...f, code: out } : f)));
    } finally {
      setFormatting(false);
    }
  }, [active, code, formatCode]);

  // Atalhos estilo VS Code
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      const mod = e.metaKey || e.ctrlKey;
      if (mod && e.key === "Enter") { e.preventDefault(); run(); return; }
      if (mod && e.shiftKey && (e.key === "F" || e.key === "f")) { e.preventDefault(); doFormat(); return; }
      if (mod && (e.key === "b" || e.key === "B")) { e.preventDefault(); setExplorerOpen((v) => !v); return; }
      if (mod && e.key === "`") { e.preventDefault(); setBottomTab((t) => (t === "terminal" ? "output" : "terminal")); return; }
      if (mod && (e.key === "s" || e.key === "S")) {
        e.preventDefault();
        const f = filesRef.current[active];
        if (f) { const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([f.code], { type: "text/x-python" })); a.download = f.name; a.click(); URL.revokeObjectURL(a.href); }
        return;
      }
    },
    [run, doFormat, active],
  ...
  const extensions = useMemo(() => {
    const ext: any[] = [python()];
    if (minimap) {
      const create = () => { const dom = document.createElement("div"); return { dom }; };
      ext.push(showMinimap.compute([], () => ({ create, displayText: "blocks", showOverlay: "always" })) as any);
    }
    return ext;
  }, [minimap]);
  ...
  if (fullscreen) {
    ...
    const ACT = [{ id: "explorer", icon: Files, title: "Arquivos" }, { id: "examples", icon: BookOpen, title: "Exemplos" }, { id: "search", icon: Search, title: "Buscar" }, { id: "outline", icon: ListTree, title: "Estrutura" }, { id: "snippets", icon: Code2, title: "Snippets" }, { id: "ssh", icon: Server, title: "SSH remoto" }, { id: "settings", icon: Settings, title: "Configurações" }] as const;
    // outline do arquivo ativo (funções/classes)
    const outline = cur.code.split("\n").map((ln, i) => ({ ln: i + 1, text: ln })).filter((r) => /^\s*(def|class|async def)\s+\w/.test(r.text)).map((r) => ({ ...r, name: r.text.trim().replace(/\(.*/, "").replace(/:\s*$/, ""), indent: (r.text.match(/^\s*/)?.[0].length ?? 0) }));
      ...
      <div onKeyDown={onKeyDown} className="flex h-screen w-screen flex-col overflow-hidden" style={{ background: ui.bg, color: ui.text }}>
        <div className="flex h-9 shrink-0 items-center justify-between px-3 text-xs" style={{ background: ui.title, borderBottom: `1px solid ${ui.border}` }}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={doFormat} disabled={formatting} title="Formatar código (⇧⌘F)" className="rounded p-1.5 hover:bg-white/10 disabled:opacity-50">{formatting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Wand2 className="h-4 w-4" />}</button>
            <button onClick={downloadActive} title="Baixar .py (⌘S)" className="rounded p-1.5 hover:bg-white/10"><Download className="h-4 w-4" /></button>
        ...
        <div className="flex min-h-0 flex-1">
          ...
          {explorerOpen && (
            <div className="flex w-64 shrink-0 flex-col overflow-hidden text-[13px]" style={{ background: ui.panel, borderRight: `1px solid ${ui.border}` }}>
              <div className="flex items-center justify-between px-4 py-2 text-[11px] font-semibold uppercase tracking-wide" style={{ color: ui.textDim }}>
                {panel === "explorer" ? "Arquivos" : panel === "examples" ? `Exemplos (${IDE_EXAMPLES.length})` : panel === "search" ? "Buscar" : panel === "outline" ? "Estrutura" : panel === "snippets" ? "Snippets" : panel === "ssh" ? "SSH remoto" : "Configurações"}
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                {panel === "outline" && (
                  <div className="py-1">
                    {outline.length === 0 ? (
                      <p className="px-4 py-2 text-xs" style={{ color: ui.textDim }}>Nenhuma função ou classe definida.</p>
                    ) : outline.map((o, i) => (
                      <div key={i} className="flex items-center gap-1.5 py-0.5 pr-2 text-[13px]" style={{ paddingLeft: 12 + o.indent * 6 }}>
                        {o.text.includes("class") ? <span style={{ color: ui.accent }}>◆</span> : <span style={{ color: ui.textDim }}>ƒ</span>}
                        <span className="truncate">{o.name.replace(/^(async def|def|class)\s+/, "")}</span>
                        <span className="ml-auto text-[10px]" style={{ color: ui.textDim }}>{o.ln}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {panel === "snippets" && (
                  <div className="py-1">
                    {SNIPPET_CATEGORIES.map((catg) => (
                      <div key={catg}>
                        <p className="px-3 py-1 text-[11px] font-semibold uppercase" style={{ color: ui.textDim }}>{catg}</p>
                        {SNIPPETS.filter((s) => s.category === catg).map((s) => (
                          <button key={s.title} onClick={() => setActiveCode(s.code)} className="flex w-full items-center gap-1.5 py-0.5 pl-6 pr-2 text-left text-[13px]" onMouseEnter={(e) => (e.currentTarget.style.background = ui.hover)} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                            <Code2 className="h-3.5 w-3.5 shrink-0" style={{ color: ui.accent }} /> <span className="truncate">{s.title}</span>
                          </button>
                        ))}
                      </div>
                    ))}
                ...
                {panel === "ssh" && (
                  <div className="space-y-3 p-3 text-[13px]">
                    <p className="flex items-center gap-1.5 text-xs font-semibold"><Server className="h-3.5 w-3.5" /> SSH para servidor remoto</p>
                    <p className="text-xs leading-relaxed" style={{ color: ui.textDim }}>
                      O navegador não fala SSH direto. Rode o <b>bridge</b> (WebSocket→SSH) na sua VPS e informe a URL <code>wss://…</code> abaixo. Depois, no terminal: <code>ssh usuario@host</code>.
                    </p>
                    <input
                      value={sshBridge}
                      onChange={(e) => setSshBridge(e.target.value)}
                      onBlur={() => { try { localStorage.setItem("pytrack-ssh-bridge", sshBridge); } catch {} }}
                      placeholder="wss://meuservidor.com:8022"
                      className="w-full rounded px-2 py-1.5 text-xs outline-none"
                      style={{ background: ui.hover, color: ui.text, border: `1px solid ${ui.border}` }}
                    />
                    <button onClick={() => { try { localStorage.setItem("pytrack-ssh-bridge", sshBridge); } catch {}; setBottomTab("terminal"); }} className="w-full rounded py-1.5 text-xs font-medium text-white" style={{ background: ui.accent }}>
                      Salvar e abrir terminal
                    </button>
                    <p className="text-[11px]" style={{ color: ui.textDim }}>
                      O código do bridge fica em <code>ssh-bridge/</code> no repositório. Inicie com <code>node server.js</code> na VPS.
                    </p>
                  </div>
                )}
                ...
                {panel === "settings" && (
                  <div className="space-y-4 p-3">
                    ...
                    <button onClick={() => setMinimap((m) => !m)} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs" style={{ background: ui.hover }}>
                      <MapIcon className="h-3.5 w-3.5" /> Minimapa: {minimap ? "ligado" : "desligado"}
                    </button>
                    <button onClick={doFormat} disabled={formatting} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs disabled:opacity-50" style={{ background: ui.hover }}>
                      <Wand2 className="h-3.5 w-3.5" /> {formatting ? "Formatando…" : "Formatar código (⇧⌘F)"}
                    </button>
                  </div>
                )}
              </div>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            {/* divisória arrastável p/ redimensionar o painel inferior */}
            <div
              onMouseDown={(e) => {
                e.preventDefault();
                const startY = e.clientY; const startH = termHeight;
                const move = (ev: MouseEvent) => setTermHeight(Math.max(120, Math.min(window.innerHeight - 220, startH + (startY - ev.clientY))));
            ...
            />
            <div className="flex shrink-0 flex-col" style={{ height: termHeight, background: ui.terminal, borderTop: `1px solid ${ui.border}` }}>
              <div className="flex items-center justify-between px-3 py-1 text-[11px]" style={{ color: ui.textDim, borderBottom: `1px solid ${ui.border}` }}>
                <div className="flex items-center gap-1">
                  <button onClick={() => setBottomTab("output")} className="flex items-center gap-1.5 px-2 py-1 uppercase tracking-wide" style={{ color: bottomTab === "output" ? ui.text : ui.textDim, borderBottom: bottomTab === "output" ? `1.5px solid ${ui.accent}` : "1.5px solid transparent" }}>
                    <Terminal className="h-3.5 w-3.5" /> Saída
                  </button>
                  <button onClick={() => setBottomTab("terminal")} className="flex items-center gap-1.5 px-2 py-1 uppercase tracking-wide" style={{ color: bottomTab === "terminal" ? ui.text : ui.textDim, borderBottom: bottomTab === "terminal" ? `1.5px solid ${ui.accent}` : "1.5px solid transparent" }}>
                    <SquareTerminal className="h-3.5 w-3.5" /> Terminal
                  </button>
                </div>
                <div className="flex items-center gap-2">
                  {bottomTab === "output" && <button onClick={() => setOutput([])} title="Limpar"><Trash2 className="h-3.5 w-3.5" /></button>}
                  {bottomTab === "output" && output.length > 0 && <button onClick={copyOutput}>{copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}</button>}
                </div>
              </div>
              <div className="relative min-h-0 flex-1">
                <div ref={outRef} className="absolute inset-0 overflow-auto p-3 font-mono text-[13px] leading-relaxed" style={{ display: bottomTab === "output" ? "block" : "none" }}>
                  {output.length === 0 ? (
                ...
                <div className="absolute inset-0 p-1" style={{ display: bottomTab === "terminal" ? "block" : "none" }}>
                  <IdeTerminal active={bottomTab === "terminal"} colors={{ bg: ui.terminal, text: ui.text, accent: ui.accent }} getFiles={() => filesRef.current} sshBridgeUrl={sshBridge} />
                </div>
              </div>
            </div>
          </div>
