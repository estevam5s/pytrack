#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# Cron Job 06: cron_monthly_close
# Projeto: scrum-master
# Frequência: dia 1 à meia-noite
# Agendamento cron: 0 0 1 * *
# Descrição: fechamento mensal e arquivamento
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Iniciando cron job 06: cron_monthly_close"
...
if [[ "$CODE" == "200" || "$CODE" == "201" ]]; then
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] ✅ Cron 06 executado com sucesso (HTTP $CODE)"
else
