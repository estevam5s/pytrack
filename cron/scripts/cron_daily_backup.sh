#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# Cron Job 08: cron_daily_backup
# Projeto: scrum-master
# Frequência: diário às 04:00
# Agendamento cron: 0 4 * * *
# Descrição: backup automático do banco
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Iniciando cron job 08: cron_daily_backup"
...
if [[ "$CODE" == "200" || "$CODE" == "201" ]]; then
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] ✅ Cron 08 executado com sucesso (HTTP $CODE)"
else
