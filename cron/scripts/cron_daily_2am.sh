#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# Cron Job 02: cron_daily_2am
# Projeto: scrum-master
# Frequência: diário às 02:00
# Agendamento cron: 0 2 * * *
# Descrição: backup e limpeza de dados temporários
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Iniciando cron job 02: cron_daily_2am"
...
if [[ "$CODE" == "200" || "$CODE" == "201" ]]; then
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] ✅ Cron 02 executado com sucesso (HTTP $CODE)"
else
