#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# Cron Job 05: cron_weekly_report
# Projeto: scrum-master
# Frequência: segunda às 03:00
# Agendamento cron: 0 3 * * 1
# Descrição: relatórios semanais e consolidação
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Iniciando cron job 05: cron_weekly_report"
...
if [[ "$CODE" == "200" || "$CODE" == "201" ]]; then
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] ✅ Cron 05 executado com sucesso (HTTP $CODE)"
else
