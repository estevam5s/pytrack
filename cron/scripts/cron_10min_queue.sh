#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
# Cron Job 03: cron_10min_queue
# Projeto: scrum-master
# Frequência: a cada 10 minutos
# Agendamento cron: */10 * * * *
# Descrição: processamento de filas e status
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] Iniciando cron job 03: cron_10min_queue"
...
if [[ "$CODE" == "200" || "$CODE" == "201" ]]; then
  echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] ✅ Cron 03 executado com sucesso (HTTP $CODE)"
else
