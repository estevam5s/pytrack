...
## 🛡️ Proteção contra DDoS, DoS e força bruta (Hydra)

| Ataque | Proteção na PyTrack |
|--------|---------------------|
| **DDoS / DoS** (sobrecarga) | **Vercel** absorve e mitiga ataques volumétricos na borda automaticamente (rede global + CDN). Páginas públicas usam ISR/cache, reduzindo carga no banco. **Recomendado:** ativar o **Vercel Firewall / Attack Challenge Mode** no painel ao detectar ataque, ou colocar **Cloudflare** na frente do domínio (proxy laranja) para WAF + rate limiting de rede. |
| **Força bruta / Hydra** (login) | Rate limiting **por e-mail** (8/min) **e por IP** (30/min) no login; **2FA (TOTP)** opcional; senhas com hash (Supabase); bloqueio de e-mails descartáveis e checagem de MX no cadastro. |
| **Credential stuffing** | Mesmo rate limiting + 2FA + verificação de e-mail. |
| **Abuso de cadastro** (multi-conta) | Limite de **2 contas por IP** (`signup_ips`), rate limit de cadastro (5/10min), checagem de e-mail real (MX). |
| **Abuso de API** | Rate limiting por chave (60/min) na API pública; chaves com hash sha256, revogáveis. |
| **Spam (comunidade/suporte/chat)** | Rate limiting por usuário em posts, comentários, suporte e chat; bloqueio de usuários na moderação. |
| **Vazamento de dados** | **RLS** em todas as tabelas (usuário só lê o próprio dado); `service_role` só no servidor; segredos nunca no cliente; CSP + HSTS + headers de segurança. |
| **Sequestro de conta** | 2FA, confirmação de e-mail, cookies httpOnly (Supabase SSR), CSP anti-XSS, sem tokens no cliente. |

**Camadas de defesa (defense in depth):** borda (Vercel/CDN) → rate limiting (app) → autenticação/2FA → autorização (RLS) → headers (CSP/HSTS).

**Ação recomendada para picos de ataque:** ativar **Vercel Firewall** (Project → Firewall) e, se necessário, **Cloudflare** com regras de rate limiting de rede — isso bloqueia o tráfego malicioso antes mesmo de chegar à aplicação.
